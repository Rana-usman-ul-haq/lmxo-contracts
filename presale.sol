...
contract LMXOPresale is Ownable {
    ...
    IERC20 public immutable token = IERC20(0x0000000000000000000000000000000000000000); // to be updated when token deployed
    ...
    address public wallet = payable(0xCE22eEAf565636038f05F5A87b62946993816c82);

    // How many token units a buyer gets per ETH & USDT
    uint256 public tokenPriceUSD = 15;
    uint256 public ethDivider = 10000;
    uint256 private usdtRate = 66667;
    ...
    function claimTokens() public {
      ...
      //  uint256 amount = Customer[msg.sender].tokensBought;
        ...
        if (BuyerPhase[msg.sender].phaseOne == false) {
           ...
           uint256 vestingOne = Customer[msg.sender].tokensBought.mul(5).div(100);
      ...
      if (block.timestamp >= endTime + 90 days ){ //90 days
        if (BuyerPhase[msg.sender].phaseTwo == false)  {
               ...
               uint256 vestingTwo = Customer[msg.sender].tokensBought.mul(1583).div(10000);
      ...
      if (block.timestamp >= endTime + 120 days ){ //120 days
        if (BuyerPhase[msg.sender].phaseThree == false)  {
               ...
               uint256 vestingThree = Customer[msg.sender].tokensBought.mul(1583).div(10000);
               valueToBeClaimed += vestingThree;
               Customer[msg.sender].totalAmount -= vestingThree;
        }
      }

      if (block.timestamp >= endTime + 150 days ){ //150 days
        if (BuyerPhase[msg.sender].phaseFour == false)  {
               BuyerPhase[msg.sender].phaseFour = true;
               uint256 vestingFour = Customer[msg.sender].tokensBought.mul(1583).div(10000);
               valueToBeClaimed += vestingFour;
               Customer[msg.sender].totalAmount -= vestingFour;
        }
      }

      if (block.timestamp >= endTime + 180 days ){ //180 days
        if (BuyerPhase[msg.sender].phaseFive == false)  {
               BuyerPhase[msg.sender].phaseFive = true;
               uint256 vestingFive = Customer[msg.sender].tokensBought.mul(1583).div(10000);
               valueToBeClaimed += vestingFive;
               Customer[msg.sender].totalAmount -= vestingFive;
        }
      }

      if (block.timestamp >= endTime + 210 days ){ //210 days
        if (BuyerPhase[msg.sender].phaseSix == false)  {
               BuyerPhase[msg.sender].phaseSix = true;
               uint256 vestingSix = Customer[msg.sender].tokensBought.mul(1583).div(10000);
               valueToBeClaimed += vestingSix;
               Customer[msg.sender].totalAmount -= vestingSix;
        }
      }

      if (block.timestamp >= endTime + 240 days ){ //240 days
        if (BuyerPhase[msg.sender].phaseSeven == false)  {
               BuyerPhase[msg.sender].phaseSeven = true;
               uint256 vestingSeven = Customer[msg.sender].totalAmount;
               valueToBeClaimed += vestingSeven;
